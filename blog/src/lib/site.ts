/* blog/src/lib/site.ts
 * Copyright (c) 2026 Clove Nytrix Doughmination Twilight
 * Licensed under the DASL-1.0 Licence.
 * See LICENCE.md in the project root for full licence information.
 */
/* src/lib/site.ts
 * Single source of truth for site-wide constants. Change SITE_URL here if the
 * blog moves to a different domain.
 */
export const SITE_URL = "https://doughmination.site";
export const SITE_NAME = "Clove Twilight's Blog";
export const SITE_TAGLINE = "Random Yapping and Stuff";
export const AUTHOR = "Clove Twilight";
export const AVATAR = "/assets/favicon.png";

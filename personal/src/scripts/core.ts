...
/* ===========================================================================
 * Realtime client (window.DM). One WebSocket to /v2/ws serves every page;
 * feature scripts subscribe via DM.on / DM.get / DM.request and never poll.
 * Runs after ctpClearPageState is defined above so page-scoped cleanup hooks on.
 * See REALTIME_PROTOCOL.md for the wire format.
 * =========================================================================== */
(function () {
  "use strict";
  ...
  function primeOnce(topic) {
    ...
    if (pid) {
      ...
      // Emit the flat presence (data.presence) to match the socket's shape, so
      // consumers see one shape whether it came from the socket or this fallback.
      fetchInto(topic, REST_BASE + "/discord/users/" + encodeURIComponent(pid), function (j) {
        return j && j.data && j.data.presence ? { id: pid, data: j.data.presence } : null;
      });
      return;
    }
